...
class ArmToolchainPackage(ConanFile):
    ...
    def validate(self):
        ...
        if (
            self.settings_target.os != "Linux"
            or self.settings_target.arch not in valid_archs
        ):
            raise ConanInvalidConfiguration(
                f"This toolchain only supports building for Linux-{valid_archs.join(',')}. "
        ...
        if Version(self.settings_target.compiler.version) >= Version("14") or Version(
            self.settings_target.compiler.version
        ) < Version("13"):
            raise ConanInvalidConfiguration(
    ...
    def build(self):
        ...
        get(
            self,
            f"https://developer.arm.com/-/media/Files/downloads/gnu/13.2.rel1/binrel/arm-gnu-toolchain-13.2.rel1-x86_64-{toolchain}.tar.xz",
            sha256=sha,
            strip_root=True,
        )
    ...
    def package_info(self):
        ...
        self.cpp_info.bindirs.append(
            os.path.join(self.package_folder, toolchain, "bin")
        )
